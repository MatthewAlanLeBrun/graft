defmodule Graft.ServerTest do
    ...
    test "updates term and demotes when term > current_term" do
        ...
        assert ^new_data_ae = ^new_data_ae_reply = ^new_data_rv = ^new_data_rv_reply = %Graft.State{
            me: {:test_server, :nonode@nohost},
            current_term: 10,
            leader: server
        }
        ...
        {:next_state, :follower, new_data_ae_reply, _} = Graft.Server.candidate :cast, ae_reply, state
        {:next_state, :follower, new_data_rv, _} = Graft.Server.candidate :cast, rv, state
        {:next_state, :follower, new_data_rv_reply, _} = Graft.Server.candidate :cast, rv_reply, state
    ...
    test "candidate updates state on conversion" do
        ...
        assert_receive {_, %Graft.RequestVoteRPC{
            term: 1,
